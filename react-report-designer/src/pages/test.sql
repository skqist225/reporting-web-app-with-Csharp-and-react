...
SELECT SINHVIEN.HO, SINHVIEN.TEN, MONHOC.TENMH, LOPTINCHI.MAMH, LOP.TENLOP, KHOA.TENKHOA 
FROM (SELECT SINHVIEN.HO, SINHVIEN.TEN, SINHVIEN.MALOP FROM  SINHVIEN) AS SINHVIEN  
JOIN  (SELECT LOP.TENLOP, LOP.MALOP, LOP.MAKHOA FROM   LOP) AS LOP  ON SINHVIEN.MALOP = LOP.MALOP 
JOIN  (SELECT KHOA.TENKHOA, KHOA.MAKHOA FROM  KHOA) AS KHOA  ON LOP.MAKHOA = KHOA.MAKHOA
JOIN (SELECT LOPTINCHI.MAMH, LOPTINCHI.MAKHOA FROM  LOPTINCHI) AS LOPTINCHI  ON LOPTINCHI.MAKHOA = KHOA.MAKHOA
JOIN  (SELECT MONHOC.TENMH, MONHOC.MAMH FROM  MONHOC) AS MONHOC  ON LOPTINCHI.MAMH = MONHOC.MAMH  

SELECT SINHVIEN.HO, SINHVIEN.TEN, MONHOC.TENMH, LOPTINCHI.MAMH, LOP.TENLOP, KHOA.TENKHOA 
FROM (SELECT SINHVIEN.HO, SINHVIEN.TEN, SINHVIEN.MALOP FROM  SINHVIEN) AS SINHVIEN  
JOIN  (SELECT LOP.TENLOP, LOP.MALOP, LOP.MAKHOA FROM   LOP) AS LOP  ON SINHVIEN.MALOP = LOP.MALOP 
, (SELECT LOPTINCHI.MAMH, LOPTINCHI.MAKHOA FROM  LOPTINCHI) AS LOPTINCHI  
JOIN  (SELECT MONHOC.TENMH, MONHOC.MAMH FROM  MONHOC) AS MONHOC  ON LOPTINCHI.MAMH = MONHOC.MAMH  
JOIN  (SELECT KHOA.TENKHOA, KHOA.MAKHOA FROM  KHOA) AS KHOA  ON LOPTINCHI.MAKHOA = KHOA.MAKHOA  
JOIN  (SELECT KHOA.TENKHOA, KHOA.MAKHOA FROM  KHOA) AS KHOA  ON LOP.MAKHOA = KHOA.MAKHOA

SELECT SINHVIEN.HO, MONHOC.TENMH, LOPTINCHI.NIENKHOA, LOP.TENLOP, KHOA.TENKHOA 
FROM (SELECT SINHVIEN.HO, SINHVIEN.MALOP FROM  SINHVIEN) AS SINHVIEN  
JOIN  (SELECT LOP.TENLOP, LOP.MALOP, LOP.MAKHOA FROM   LOP) AS LOP  ON SINHVIEN.MALOP = LOP.MALOP  
JOIN  (SELECT KHOA.TENKHOA, KHOA.MAKHOA FROM  KHOA) AS KHOA  ON LOP.MAKHOA = KHOA.MAKHOA ,  
JOIN (SELECT LOPTINCHI.NIENKHOA, LOPTINCHI.MAMH, LOPTINCHI.MAKHOA FROM   LOPTINCHI) AS LOPTINCHI ON LOPTINCHI.MAKHOA = KHOA.MAKHOA  
JOIN  (SELECT MONHOC.TENMH, MONHOC.MAMH FROM  MONHOC) AS MONHOC  ON LOPTINCHI.MAMH = MONHOC.MAMH